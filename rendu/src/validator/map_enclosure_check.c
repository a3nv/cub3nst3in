...
/// @brief Get Cell Safe (gcs) Get the character
/// at the specified row and column in the map
...
/// @brief check if the cells are adjacent to a '0' are either '1' or '0'
/// and zero is not on the edge
/// @param g
/// @param row
/// @param col
/// @param mapcheck
...
/// @note The function checks if the map is
/// closed by checking if the cells one by one
...
bool	check_enclosure(t_game *g)
{
	...
	while (g->map->data[row] && g->map->data[row][col])
	{
		while (g->map->data[row][col])
		{
			...
			{
				debug("[NOTE] problematic symbol encoutered @map[%i][%i]: <%c>",
